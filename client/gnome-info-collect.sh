...

{
    ...
    hostnamectl | grep -e 'Hardware' -e 'Operating System' | \
        sed -e 's/^[ \t]*/"/g' -e 's/:[ \t]*/":"/g' -e 's/$/",/g' # Convert to JSON format

    #~ Get GPU info and driver version
    # glxinfo | grep -e "Vendor" -e "Device" -e "Version" | \
    #     sed -e 's/^[ \t]*/"/g' -e 's/:[ \t]*/":"/g' -e 's/$/",/g' | \
    #     sed -e 's/Vendor/GPU Vendor/' -e 's/Device/GPU Device/' -e 's/Version/GPU Driver version/'
